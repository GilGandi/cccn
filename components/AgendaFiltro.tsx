...
export default function AgendaFiltro({ eventos, categorias }: { eventos: Evento[]; categorias: Categoria[] }) {
  ...
  const filtrados = filtro === 'todos'
    ? eventos
    : eventos.filter(ev => ev.categoria?.id === filtro)
  ...
  return (
    <div className="max-w-[1200px] mx-auto px-6 sm:px-10 lg:px-16 py-12">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          ...
          {categorias.map(c => (
            <button
              ...
              style={{
                background: filtro === c.id ? c.cor : 'transparent',
                color: filtro === c.id ? '#0a0a0a' : c.cor,
      ...
      {/* Grid */}
      {filtrados.length === 0 ? (
        <p className="font-body text-[0.88rem] text-[#888480] py-16 text-center">Nenhum evento encontrado para este filtro.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3"
          ...
          {filtrados.map(ev => {
            ...
            const mes = data.toLocaleDateString('pt-BR', { month: 'short', weekday: 'short' })
            return (
              <div key={ev.id} className="relative p-6 bg-[#0a0a0a]">
                {ev.categoria && (
                  <span className="inline-block font-body text-[0.58rem] tracking-[0.2em] uppercase px-2.5 py-1 mb-4 rounded"
                    style={{ background: ev.categoria.cor + '22', color: ev.categoria.cor, border: `1px solid ${ev.categoria.cor}55` }}>
                    {ev.categoria.nome}
                  </span>
                )}
                <div className="absolute top-6 right-6 font-body text-[0.62rem] text-[#888480]">{ev.horario}</div>
